...
class Search(Base):
    URI = '/search/!'
    def images(self, keywords, **kwargs):
        url = '%s/image' % keywords
        params = kwargs
        params['identifier'] = keywords

        response = self.make_request(url, base_uri='/search/', params=dict(params), auth=NOT_REQUIRED)
        content = self.parse_response(response.content, 'json')

        return self.get_images_in_response(content)

    def get_images_in_response(self, response_content):
        result = response_content.get('content').get('result')
        images = result.get('primary').get('media')
        attributes = result.get('_attribs')
